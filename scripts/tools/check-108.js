...
async function main() {
  const result = await db.query("SELECT lead_website, lead_name, qscore, full_analysis FROM lead_analyses WHERE lead_website LIKE '%108mmhouse%'");
  ...
  if (result.rows.length > 0) {
    ...
    console.log('Lead 108mmhouse not found in database.');
  }
}
